...
describe('src/Core', function () {
  ...
  describe('adding a file', function () {
    ...
    it('should call utils.generatePreview when file-added is triggered and thumbnail generation is allowed', function () {
      ...
      expect(_Utils2.default.createThumbnail).toHaveBeenCalledTimes(1);
      expect(_Utils2.default.createThumbnail.mock.calls[0][1]).toEqual(280);
    });

    it('should return an object url of the image when file-added is triggered and thumbnail generation is disabled', function () {
      var core = new _Core2.default({
  ...
  describe('uploading a file', function () {
    it('should return a { successful, failed } pair containing file objects', function () {
      ...
      return Promise.all([core.addFile({ source: 'jest', name: 'foo.jpg', type: 'image/jpeg', data: new Uint8Array() }), core.addFile({ source: 'jest', name: 'bar.jpg', type: 'image/jpeg', data: new Uint8Array() })]).then(function () {
  ...
  describe('checkRestrictions', function () {
    it('should enforce the maxNumberOfFiles rule', function () {
      ...
      return expect(core.addFile({
        ...
        type: 'image/jpeg',
        data: _Utils2.default.dataURItoFile(sampleImageDataURI, {})
      })).rejects.toMatchObject(new Error('File not allowed')).then(function () {
